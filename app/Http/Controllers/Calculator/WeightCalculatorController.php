...
class WeightCalculatorController extends Controller
{
    ...
    /**
     * Return latest market rate (mocked service)
     */
    public function marketRate(Request $request): JsonResponse
    {
        try {
            $metal = $request->input('metal', 'gold');
            $data = $this->marketRateService->getLatestRate($metal);
    ...
    {
        try {
            $calculationDetails = $request->input('calculation_details');
            ...
            $calculation = WeightCalculation::create([
                ...
                'stone_unit' => $request->input('stone_unit', 'carat'),
                'stone_price_per_carat' => $request->input('stone_price_per_carat', 0),
                'tax_percentage' => $request->input('tax_percentage', 0),
                'discount_percentage' => $request->input('discount_percentage', 0),
                'ratti_type' => $request->input('ratti_type', 'sunari'),
                'custom_charges' => $request->input('custom_charges', 0),
    ...
    public function exportPDF($id)
    {
        try {
            ...
            $pdf = \PDF::loadView('calculator.print', [
                'calculation' => $calculation,
                'details' => $details,
            ]);
    ...
    /**
     * Return calculation payload for POS usage
     */
    public function useInPOS(Request $request): JsonResponse
    {
        try {
            $id = $request->input('calculation_id');
            if (! $id) {
                return response()->json(['error' => 'calculation_id required'], 422);
            }

            $calculation = WeightCalculation::findOrFail($id);

            // Basic authorization: only owner can use
            if ($calculation->user_id !== auth()->id()) {
                return response()->json(['error' => 'Unauthorized'], 403);
            }
