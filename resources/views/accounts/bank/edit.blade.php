    ...
    <div class="row align-items-center mb-5">
        <div class="col-md-6">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h1 class="h2 mb-1 text-dark fw-800">Edit Account</h1>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active fw-600 text-secondary" aria-current="page">Modify Vault</li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <div class="col-md-6 text-md-end mt-3 mt-md-0">
            <span class="badge bg-gold text-dark px-3 py-2 rounded-pill fw-700 shadow-gold">
                <i class="fas fa-edit me-1"></i>Update Entity
            </span>
        </div>
    </div>
            ...
            <div class="card border-0 shadow-soft rounded-24 overflow-hidden mb-4">
                <div class="card-header bg-premium-dark p-4 border-0">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h5 class="mb-0 fw-800 text-white">{{ $account->bank_name }}</h5>
                            <p class="text-white-50 small mb-0 fw-600">A/C: {{ $account->account_number }}</p>
                        </div>
                    </div>
                </div>
                <div class="card-body p-5">
                    <form action="{{ route('accounts.bank-payments.accounts.update', $account->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group glass-input-group shadow-sm rounded-15">
                                    ...
                                    <input type="text" name="bank_name" class="form-control border-0 py-3 fw-600 @error('bank_name') is-invalid @enderror" 
                                           placeholder="e.g. HBL Bank" value="{{ old('bank_name', $account->bank_name) }}" required>
                                </div>
                                @error('bank_name')
                                    <div class="text-danger smaller fw-700 mt-2 px-2">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group glass-input-group shadow-sm rounded-15">
                                    ...
                                    <input type="text" name="ifsc_code" class="form-control border-0 py-3 fw-600" 
                                           placeholder="e.g. HBL0001" value="{{ old('ifsc_code', $account->ifsc_code) }}">
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group glass-input-group shadow-sm rounded-15">
                                    ...
                                    <input type="text" name="account_name" class="form-control border-0 py-3 fw-600 @error('account_name') is-invalid @enderror" 
                                           placeholder="e.g. Main Business A/C" value="{{ old('account_name', $account->account_name) }}" required>
                                </div>
                                @error('account_name')
                                    <div class="text-danger smaller fw-700 mt-2 px-2">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group glass-input-group shadow-sm rounded-15">
                                    ...
                                    <input type="text" name="account_number" class="form-control border-0 py-3 fw-600 @error('account_number') is-invalid @enderror" 
                                           placeholder="XXXX-XXXX-XXXX" value="{{ old('account_number', $account->account_number) }}" required>
                                </div>
        ...
        <div class="col-lg-4">
            <div class="stat-card border-0 p-4 rounded-24 shadow-soft mb-4">
                <h6 class="fw-800 text-dark mb-3 d-flex align-items-center">
                    <i class="fas fa-info-circle text-gold me-2"></i>Note
                </h6>
                <p class="text-muted smaller fw-600 line-height-relaxed mb-0">
                    Changing the account number or bank name may affect how historical transactions are identified in reports. Proceed with caution.
                </p>
            </div>

            <div class="card border-0 shadow-premium rounded-24 overflow-hidden bg-premium-dark text-white p-4">
                <h6 class="fw-800 mb-4 text-gold">Current Status</h6>
                <div class="mb-3">
                    <p class="text-white-50 small mb-1 fw-600">Available Balance</p>
                    <h4 class="fw-800 text-white mb-0">Rs.{{ number_format($account->current_balance, 2) }}</h4>
                </div>
                <div class="p-2 {{ $account->is_active ? 'bg-success-soft text-success' : 'bg-danger-soft text-danger' }} rounded-8 text-center fw-800 smaller">
                    {{ $account->is_active ? 'ACTIVE VAULT' : 'INACTIVE' }}
                </div>
            </div>
        </div>
    </div>
